 ...
 *
 * Диск Render эфемерный и стирается при каждом перезапуске, поэтому кроме локального файла
 * список уходит резервной копией через хук backup (бот держит её закреплённым документом
 * в чате администратора), а при старте поднимается обратно через restore().
 */
